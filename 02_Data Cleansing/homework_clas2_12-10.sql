...
ALTER TABLE `empleado` ADD `Salario` DECIMAL(10,2) NOT NULL DEFAULT '0' AFTER `Salarios`;
UPDATE `empleado` SET Salario = REPLACE(Salarios,',','.');
ALTER TABLE `empleado` DROP `Salarios`;


ALTER TABLE `producto` ADD `Precio` DECIMAL(15,3) NOT NULL DEFAULT '0' AFTER `Precios`;
UPDATE `producto` SET Precio = REPLACE(Precios,',','.');
ALTER TABLE `producto` DROP `Precios`;


ALTER TABLE `sucursal` 	ADD `Latitud` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Longitudes`, 
						ADD `Longitud` DECIMAL(13,10) NOT NULL DEFAULT '0' AFTER `Latitudes`;
UPDATE `sucursal` SET Latitud = REPLACE(Latitudes,',','.');
UPDATE `sucursal` SET Longitud = REPLACE(Longitudes,',','.');
SELECT * FROM `sucursal`;
ALTER TABLE `sucursal` DROP `Latitudes`;
ALTER TABLE `sucursal` DROP `Longitudes`;


UPDATE `venta` set `Precio` = 0 WHERE TRIM(`Precio`) = "";
...
/*11) Normalizacion tabla empleado*/
